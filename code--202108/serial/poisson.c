...
void matrix_stencil_mode_const(Stencil *matrix, double *diag,
    ...
    for (i = 1; i < size - 1; i++) {
        ...
        num = i;
        diag[num] = d;
        initial_stencil(&matrix[num], -k1, -k1, -k2, 0);

        //north
        num = size * size - size + i;
        diag[num] = d;
        initial_stencil(&matrix[num], -k1, -k1, 0, -k2);
        ...
        num = size * i;
        diag[num] = d;
        initial_stencil(&matrix[num], 0, -k1, -k2, -k2);

        //east
        num = size * i + size - 1;
        diag[num] = d;
        initial_stencil(&matrix[num], -k1, 0, -k2, -k2);
    }
    ...
    //W-S
    num = 0; diag[num] = d;
    initial_stencil(&matrix[num], 0, -k1, -k2, 0);
    //E-S
    num = size - 1; diag[num] = d;
    initial_stencil(&matrix[num], -k1, 0, -k2, 0);
    
    //W-N
    num = size * size - size; diag[num] = d;
    initial_stencil(&matrix[num], 0, -k1, 0, -k2);
    //E-N
    num = size * size - 1; diag[num] = d;
    initial_stencil(&matrix[num], -k1, 0, 0, -k2);
    ...
    for (j = 1; j < size - 1; j++) {
        for (i = 1; i < size - 1; i++) {
            ...
            num = j * size + i;
            diag[num] = d;
            initial_stencil(&matrix[num], -k1, -k1, -k2, -k2);
        }
    }
...
void matrix_stencil_mode(Stencil *matrix, double *diag,
    ...
    for (i = 1; i < size - 1; i++) {
        ...
        num = i;
        diag[num] = d;
        initial_stencil(&matrix[num], -k1_w, -k1_e, -k2_n, 0);
        ...
        num = size * size - size + i;
        diag[num] = d;
        initial_stencil(&matrix[num], -k1_w, -k1_e, 0, -k2_s);
        ...
        num = size * i;
        diag[num] = d;
        initial_stencil(&matrix[num], 0, -k1_e, -k2_n, -k2_s);
        ...
        num = size * i + size - 1;
        diag[num] = d;
        initial_stencil(&matrix[num], -k1_w, 0, -k2_n, -k2_s);
    }
    ...
    num = 0; diag[num] = d;
    initial_stencil(&matrix[num], 0, -k1_e, -k2_n, 0);
    //E-S -- i = size - 1; j = 0
    ...
    num = size - 1; diag[num] = d;
    initial_stencil(&matrix[num], -k1_w, 0, -k2_n, 0);
    ...
    num = size * size - size; diag[num] = d;
    initial_stencil(&matrix[num], 0, -k1_e, 0, -k2_s);
    ...
    num = size * size - 1; diag[num] = d;
    initial_stencil(&matrix[num], -k1_w, 0, 0, -k2_s);
    ...
    for (j = 1; j < size - 1; j++) {
        for (i = 1; i < size - 1; i++) {
            ...
            num = j * size + i;
            diag[num] = d;
            initial_stencil(&matrix[num], -k1_w, -k1_e, -k2_n, -k2_s);
        }
    }
