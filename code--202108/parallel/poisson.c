...
void matrix_stencil_mode(Stencil *matrix, double *diag, double (*k1)(double, double), 
    ...
    for (j = 0; j < nly; j++) {
        ...
        for (i = 0; i < nlx; i++) {
            ...
            num_l = j * nlx + i;
            diag[num_l] = d;
            initial_stencil(&matrix[num_l], -k1_w, -k1_e, -k2_n, -k2_s);
        }
    }
    ...
    if (myidx == 0) {
    	...
    	for (j = 0; j < nly; j++) {
	        ...
	        num_l = nlx * j;
	        diag[num_l] = d;
	        initial_stencil(&matrix[num_l], 0, -k1_e, -k2_n, -k2_s);
    	}
    }
    if (myidx == npx - 1) {
    	//east
    	for (j = 0; j < nly; j++) {
	        ...
	        num_l = nlx * j + nlx - 1;
	        diag[num_l] = d;
	        initial_stencil(&matrix[num_l], -k1_w, 0, -k2_n, -k2_s);
    	}
    }
    if (myidy == 0) {
    	//south
    	for (i = 0; i < nlx; i++) {
	        ...
	        num_l = i;
	        diag[num_l] = d;
	        initial_stencil(&matrix[num_l], -k1_w, -k1_e, -k2_n, 0);
    	}
    }
    if (myidy == npy - 1) {
    	//north
    	for (i = 0; i < nlx; i++) {
	        ...
	        num_l = nlx * nly - nlx + i;
	        diag[num_l] = d;
	        initial_stencil(&matrix[num_l], -k1_w, -k1_e, 0, -k2_s);
    	}
    }
    ...
    if (myidx == 0 && myidy == 0) {
	    ...
	    num_l = 0;
	    diag[num_l] = d;
	    initial_stencil(&matrix[num_l], 0, -k1_e, -k2_n, 0);    	
    }
    if (myidx == 0 && myidy == npy - 1) {
		...
		num_l = nlx * nly - nlx;
		diag[num_l] = d;
		initial_stencil(&matrix[num_l], 0, -k1_e, 0, -k2_s);
    }
    if (myidx == npx - 1 && myidy == 0) {
	    ...
	    num_l = nlx - 1;
	    diag[num_l] = d;
	    initial_stencil(&matrix[num_l], -k1_w, 0, -k2_n, 0);       	
    }
    if (myidx == npx - 1 && myidy == npy - 1) {
	    ...
	    num_l = nlx * nly - 1;
	    diag[num_l] = d;
	    initial_stencil(&matrix[num_l], -k1_w, 0, 0, -k2_s);       	
    }
