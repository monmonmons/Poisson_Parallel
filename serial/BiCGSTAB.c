...
void P_BICGSTAB(Stencil *A, double *diag, double *rhs, double *x, int precond, int precond_iter,
    ...
    for (nits = 1; nits <= maxit; nits++) {
	    ...
	    switch (precond) {
	        ...
	        case 2: Precond_CG(A, diag, d, y, EPSILON, size, precond_iter);  break;
	    }
	    ...
	    switch (precond) {
	        ...
	        case 2: Precond_CG(A, diag, s, z, EPSILON, size, precond_iter);  break;
	    }
...
//     printf("0 : \t%.8e\n", residual0);
//     printf("---------------------------------------------------\n");
//     rho_1 = 1;  alpha = 1;  omega = 1;

//     for (nits = 1; nits <= maxit; nits++) {
//     	rho_2 = dot(res, rhs, mesh_size);
